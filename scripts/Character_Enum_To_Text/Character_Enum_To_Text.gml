...
function Get_Class_Color(style)
{
		...
		if DayTime=1{
switch(style){
	case furn_style.lodge: ret=c_style_dk_orange break;
	case furn_style.regal: ret=c_regal break;
	case furn_style.sci_fi: ret=c_style_dk_blue  break;
	case furn_style.arcane: ret=c_style_dk_red break;
	case furn_style.wild: ret=c_style_dk_green break;

}
		}
		else{
switch(style){
	case furn_style.lodge: ret=c_neutral break;
	case furn_style.regal: ret=c_regal break;
	case furn_style.sci_fi: ret=c_scholarly  break;
	case furn_style.arcane: ret=c_spirit break;
	case furn_style.wild: ret=c_wild break;
...
function Furn_Class_Enum_To_Text(class)
{
	ret=""
switch(class){
	case furn_style.lodge: ret="Neutral" break;
	case furn_style.regal: ret="Regal" break;
	case furn_style.sci_fi: ret="Scholarly" break;
	case furn_style.arcane: ret="spirit" break;
	case furn_style.wild: ret="Wild" break;
}
return ret
}
...
//neutral gnomes doesnt like regal	
	Add_Class_Pref(char.gnomes,class_pref.pos,furn_style.lodge)
	Add_Class_Pref(char.gnomes,class_pref.pos,furn_style.wild)
	Add_Class_Pref(char.gnomes,class_pref.neg,furn_style.regal)
	
	//WILD
//wild pooka doesnt dislike anything -intro to wild
	Add_Class_Pref(char.hares,class_pref.pos,furn_style.wild)
	
	//NEUTRAL
//neutral hares doesnt dislike anything	
	Add_Class_Pref(char.hares,class_pref.pos,furn_style.lodge)


	//SPIRIT
//spirit golum give 2 against 2 -intro to spirit
	Add_Class_Pref(char.golums,class_pref.pos,furn_style.arcane)
	Add_Class_Pref(char.golums,class_pref.pos,furn_style.wild)

	Add_Class_Pref(char.golums,class_pref.neg,furn_style.sci_fi)
	Add_Class_Pref(char.golums,class_pref.neg,furn_style.regal)

//SCHOLARLY
//scholarly toads are against nothing give 2 loose nothing -intro to scholarly
	Add_Class_Pref(char.toads,class_pref.pos,furn_style.sci_fi)
	Add_Class_Pref(char.toads,class_pref.pos,furn_style.wild)


//WILD
//wild tribal wogs  are against proper
	Add_Class_Pref(char.wogs,class_pref.pos,furn_style.wild)
	Add_Class_Pref(char.wogs,class_pref.neg,furn_style.lodge)
	Add_Class_Pref(char.wogs,class_pref.neg,furn_style.regal)
	
	//CATS
//cats are against anything below proper
	Add_Class_Pref(char.wogs,class_pref.pos,furn_style.regal)
	Add_Class_Pref(char.wogs,class_pref.neg,furn_style.lodge)
	Add_Class_Pref(char.wogs,class_pref.neg,furn_style.wild)
